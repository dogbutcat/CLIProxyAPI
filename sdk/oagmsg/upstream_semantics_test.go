...
func TestUpstreamGeminiAudioTranscriptionPartsProjectAsText(t *testing.T) {
	ctx := context.Background()
	raw := []byte(`{"responseId":"gem_transcribe","modelVersion":"gemini-3.5-transcribe","candidates":[{"content":{"parts":[{"audioTranscription":{"text":"Single transcription part"}}]},"finishReason":"STOP"}],"usageMetadata":{"promptTokenCount":185,"candidatesTokenCount":8,"totalTokenCount":193}}`)

	openAI := TranslateNonStream(ctx, FormatGemini, FormatOpenAI, "runtime-model", nil, nil, raw, nil)
	if got := gjson.GetBytes(openAI, "choices.0.message.content").String(); got != "Single transcription part" {
		t.Fatalf("OpenAI content = %q, want transcription text; output=%s", got, openAI)
	}

	responses := TranslateNonStream(ctx, FormatGemini, FormatOpenAIResponse, "runtime-model", nil, nil, raw, nil)
	if got := gjson.GetBytes(responses, "output.0.content.0.text").String(); got != "Single transcription part" {
		t.Fatalf("Responses text = %q, want transcription text; output=%s", got, responses)
	}

	var state any
	stream := bytes.Join(TranslateStream(ctx, FormatGemini, FormatOpenAI, "runtime-model", nil, nil, raw, &state), nil)
	if !bytes.Contains(stream, []byte(`"content":"Single transcription part"`)) {
		t.Fatalf("OpenAI stream transcription text missing: %s", stream)
	}

	explicitText := []byte(`{"responseId":"gem_transcribe","modelVersion":"gemini-3.5-transcribe","candidates":[{"content":{"parts":[{"text":"explicit text","audioTranscription":{"text":"ignored transcription"}}]},"finishReason":"STOP"}]}`)
	explicitOut := TranslateNonStream(ctx, FormatGemini, FormatOpenAI, "runtime-model", nil, nil, explicitText, nil)
	if got := gjson.GetBytes(explicitOut, "choices.0.message.content").String(); got != "explicit text" {
		t.Fatalf("explicit text precedence = %q, want explicit text; output=%s", got, explicitOut)
	}
}
